...
class WordPress_Paywall {
    ...
    public function init_plugin() {
        ...
        if ($this->is_memberpress_active()) {
            ...
            $this->simple_log("MemberPress not detected, using standard content filter");
            
            // If no MemberPress, we'll handle content restriction ourselves
            add_filter('the_content', array($this, 'maybe_restrict_content'), 10);
        }
    }
    ...
    public function handle_unlock() {
        ...
        if (!$this->paywall_enabled) {
            wp_send_json_error(array('message' => 'Paywall is disabled'));
            return;
        }
        ...
        $has_real_access = isset($_POST['has_real_access']) && $_POST['has_real_access'] === '1';
        ...
        if (!$has_real_access) {
            $this->simple_log("Unlock request without real access for post $post_id, rejecting");
            wp_send_json_error(array(
        ...
        if ($this->has_unlock_access($post_id)) {
            $this->simple_log("Post $post_id already unlocked, ignoring duplicate request");
    ...
    private function store_unlock_in_cookie($post_id) {
        ...
        setcookie($cookie_name, json_encode($unlocked_posts), time() + (86400 * 30), $path, $domain, $secure, $http_only);
        $_COOKIE[$cookie_name] = json_encode($unlocked_posts); // Also update the current request
        $this->simple_log("Stored unlock in cookie for post: $post_id, value: " . json_encode($unlocked_posts));
    }
    
    /**
     * Check if a post has been unlocked
     * 
     * @param mixed $post_input Could be post ID or WP_Post object
     * @return bool
     */
    private function has_unlock_access($post_input) {
    ...
    public function maybe_bypass_memberpress_redirect($redirect, $url, $delim) {
        ...
        global $post;
        $post_id = isset($post->ID) ? $post->ID : 0; // Get ID from global $post
    ...
    public function maybe_unrestrict_content_check($is_restricted, $post_id) {
        ...
        $post_id = $this->get_post_id_from_input($post_id); // Ensure we have an integer ID
        if (!$post_id) {
            return $is_restricted;
        }
    ...
    public function maybe_remove_unauthorized_message($message, $post_id) {
        ...
        // Use the helper function to ensure we have an integer ID
        $post_id_int = $this->get_post_id_from_input($post_id);
        if (!$post_id_int) {
            return $message;
        }
        
        if ($this->has_unlock_access($post_id_int)) {
            $this->simple_log("Removing unauthorized message for post: $post_id_int");
    ...
    public function maybe_force_content_display($content, $post_id) {
        ...
        $post_id_int = $this->get_post_id_from_input($post_id); // Ensure we have an integer ID
        if (!$post_id_int) {
            return $content;
        }
        ...
        if ($this->has_unlock_access($post_id_int)) {
            ...
            if ($post) {
                $this->simple_log("Forcing display of full content for post: $post_id_int");
    ...
    public function force_process_shortcodes($process, $post_id) {
        ...
        if ($this->has_unlock_access($post_id_int)) {
            $this->simple_log("Forcing shortcode processing for post: $post_id_int");
